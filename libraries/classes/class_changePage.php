...
class changePage{
	...
	function changePage($name,$nb_items,$target_file,$path_img,$max_items_by_page,$default_item=0)
	{
  ...
  function display_navigator($text_no_item,$textNavigator='')
  {?>
    <table class="PageChange">
    <tr>
    <td class="cell"><?php
     if ($_SESSION[$this->name]>=1)
     {?>
        <a href="<?php echo add_param_url($this->target_file,$this->name.'='.max($_SESSION[$this->name]-$this->max_items_by_page,0));?>" class="std_link"><img src="<?php echo $this->path_img;?>rewind.png" alt="Fast rewind" border="0"/></a> <?php
     }	?>
    </td>
    <td class="cell">	  <?php
     if ($_SESSION[$this->name]>=1)
     {?>
        <a href="<?php echo add_param_url($this->target_file,$this->name.'='.($_SESSION[$this->name]-1))?>" class="std_link"><img src="<?php echo $this->path_img?>slow_rewind.png" alt="View previous set" border="0"/> </a><?php
     }?> 
	 </td>
	 <td class="textNavigator"><?php echo $textNavigator;?></td>
     <td class="cell"><?php 
	if ($_SESSION[$this->name]<($this->nb_items-$this->max_items_by_page))
	{?>				
	  <a href="<?php echo add_param_url($this->target_file,$this->name.'='.($_SESSION[$this->name]+1));?>" class="std_link"> <img src="<?php echo $this->path_img;?>play.png" alt="View next set" border="0" /></a><?php  			
	}?>
      </td>
      <td class="cell">	  <?php
        if ($_SESSION[$this->name]<($this->nb_items-$this->max_items_by_page))
        {?>
          <a href="<?php echo add_param_url($this->target_file,$this->name.'='.min($_SESSION[$this->name]+$this->max_items_by_page,$this->nb_items-$this->max_items_by_page))?>" class="std_link"><img src="<?php echo $this->path_img;?>fast-forward.png" alt="Fast forward" border="0"/></a> <?php
        }?>	  
	</td>
    </tr>
    </table>
