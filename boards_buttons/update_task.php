...
<div id="previewNote" class="note <?php echo $item->color;?>" style="width:<?php echo $itemWidth;?>px;height:<?php echo $itemHeight;?>px;left:0;top:45px;z-index:1">
	    <div class="item-header">
		  <span class="postit_id bloc-header"><?php echo $objectTag.($item->task_us_number.'.'.$item->task_number);?></span>
		  <span class="task_load"><?php echo $item->task_load;?></span>
		</div>
	...
	<div class="item-footer">
	  <span class="load_spent"><?php echo $item->task_done;?></span>
	  <span class="load2finish" id="preview_load2finish"><?php echo $item->task_raf;?></span>
  </div>		
...
<form action="<?php echo $targetFileAction;?><?php echo (isset($_REQUEST['blockId']) ? '&a='.md5(time()).'#Block'.$_REQUEST['blockId'] : '');?>" method="post" class="note-form" name="form_task" enctype="multipart/form-data">
...
<input type="text" <?php if ($item->status!=$boardInitialStatusTag) { echo 'readonly="readonly"';} else { echo 'onKeyUp="javascript:updateRAF();"';} ?> name="note-task_load" id="note-task_load" class="pr-task_load <?php if ($item->status!=$boardInitialStatusTag) { echo 'readonly_form_field';}?>" size="5" maxlength="5" value="<?php echo $item->task_load;?>" /> <?php echo lcfirst($text_hours);?>
<div class="clearleft"></div>
<span class="labelSpan length100"><?php echo $text_load_spent;?></span>
<input type="text" <?php if ($item->status==$boardInitialStatusTag) { echo 'readonly="readonly"';}?> name="note-load_spent" id="note-load_spent" class="pr-load_spent <?php if ($item->status==$boardInitialStatusTag) { echo 'readonly_form_field';}?>" size="5" maxlength="5" value="<?php echo $item->task_done;?>" /> <?php echo lcfirst($text_hours);?>
<div class="clearleft"></div>
<span class="labelSpan length100"><?php echo $text_load2finish;?></span>
<input type="text" <?php if ($item->status==$boardInitialStatusTag || ($item->status==$boardFinalStatusTag && $currentProject->paramBlockRaf==1)) { echo 'readonly="readonly"';}?> name="note-load2finish" id="note-load2finish" class="pr-load2finish <?php if ($item->status==$boardInitialStatusTag || ($item->status==$boardFinalStatusTag && $currentProject->paramBlockRaf==1)) { echo 'readonly_form_field';}?>" size="5" maxlength="5" value="<?php echo $item->task_raf;?>" /> <?php echo lcfirst($text_hours);?>
<div class="clearleft"></div>
<label for="note-link" ><?php echo $text_link;?></label>
<input id="note-link" type="text" size="43" name="note_link" value="<?php echo $item->task_link;?>">
<div class="clearleft"></div>
