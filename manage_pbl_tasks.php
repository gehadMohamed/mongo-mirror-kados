...
$firstColumnTitle='Product Backlog';
...
if ($currentProject->level==1)
{
  $enableFirstColumnDeleteFunction=true;
  $enableFirstColumnUpdateFunction=true;
  $simpleItemParentType='us';
  // Get Deck actions
  $deckIsAvailable=true;
  $displayMyTasks=true;  
  $usFieldForOrderMatrix='ypos';  
  require_once $pathBase.'deck_management.php';
  displayButton($button_add_user_story,$pathImages.'app/new_user_story.png',$pathBase.'boards_buttons/add_us.php?item_type=us_project&us_from='.($itemType=='us_release' ? 'release' : ''),'addButtonGeneric');	
}  
else
{
  // 
  ...
  $request=new requete("SELECT us_id AS parent_id,us_number AS display_number,text,status AS header_stamp,color,business_value AS infoBottomLeft,complexity AS infoBottomRight 
                        FROM kados_user_stories 
		        WHERE active=1 AND us_project_id_fk=".$_SESSION['current_project_id']." 
			  AND us_release_id_fk=0 
			  ".$clauseWhereUsFilter." 
			ORDER BY zpos_r ASC",$cnx->num);
...
if (count($MultipleItemsArray)!=0 || $currentProject->level==1)
{
...
else
{
  ...
  $arrayStatus[]['colName']='Product Backlog';
